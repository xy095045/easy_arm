...
/**
 * @brief submodule to measure time
 *
 */
...
typedef struct nnctrl_ctx_s {
    ...
    float classnet_feature[class_num];
} nnctrl_ctx_t;
...
static int init_param(mtcnn_ctx_t *mtcnn_ctx)
{
    ...
    strcpy(nnctrl_ctx->PNet[eClassNet].net_in_name, "0");
    strcpy(nnctrl_ctx->PNet[eClassNet].net_out_name, "192");
    strcpy(nnctrl_ctx->PNet[eClassNet].net_file, "./classnet.bin"); 
...
int nnctrl_run_classnet(mtcnn_ctx_t *mtcnn_ctx)
{
    int rval = 0;
    nnctrl_ctx_t *nnctrl_ctx = &mtcnn_ctx->nnctrl_ctx;

    rval = nnctrl_run_net(nnctrl_ctx->PNet[eClassNet].net_id, &nnctrl_ctx->PNet[eClassNet].result, NULL, NULL, NULL);
    ...
    if (nnctrl_ctx->cache_en) {
        cavalry_sync_cache(nnctrl_ctx->PNet[eClassNet].net_m.mem_size, nnctrl_ctx->PNet[eClassNet].net_m.phy_addr, 0, 1);
    }
    
    float *score_addr = (float *)(nnctrl_ctx->PNet[eClassNet].net_m.virt_addr
        + nnctrl_ctx->PNet[eClassNet].net_out.out_desc[0].addr - nnctrl_ctx->PNet[eClassNet].net_m.phy_addr);

    for (int i=0; i < class_num; i++) {
        nnctrl_ctx->classnet_feature[i] = DIM1_DATA(score_addr, i);
    }
...
void preprocess(mtcnn_ctx_t *mtcnn_ctx, cv::Mat &srcRoI, int netId)
{
    ...
    for (int i=0; i<3; i++)
    {
        memcpy(nnctrl_ctx->PNet[netId].net_in.in_desc[0].virt + i * height * width, channel_s[2-i].data, height * width); // bgr2rgb
    }
...
int postprocess(mtcnn_ctx_t *mtcnn_ctx)
{
    nnctrl_ctx_t *nnctrl_ctx = &mtcnn_ctx->nnctrl_ctx;
    float* output = nnctrl_ctx->classnet_feature;
    ...
    // auto max_ind = std::max_element(output, output + class_num - 1);
    // printf("%d\n",*std::max_element(output, output + class_num - 1));  
    // std::cout << output[9] << std::endl;
    // int class_idx = std::distance(output, max_ind);

    return class_idx;
}

std::string predict_classnet(mtcnn_ctx_t *mtcnn_ctx, cv::Mat &src_img, int img_idx)
{
    int rval = 0;

    // preprocess input to the network
    preprocess(mtcnn_ctx, src_img, eClassNet);
    std::cout << "4-----model preprocess done!!!" << std::endl;

    rval = nnctrl_run_classnet(mtcnn_ctx);
    std::cout << "5-----model run done!!!" << std::endl; 

    int class_idx;
    class_idx = postprocess(mtcnn_ctx);

	std::stringstream index;
	index << img_idx;

    std::stringstream class_index;
	class_index << class_idx;

    cv::putText(src_img, class_index.str(), cv::Point2d(20, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0,255,0), 2);
	cv::imwrite("result/result_00" + index.str() + ".jpg", src_img);

    return class_index.str();
}
...
int main()
{
	...
	for (unsigned int img_idx = 0; img_idx < images.size(); img_idx++) {
        ...
        idx_result = predict_classnet(&mtcnn_ctx, img, img_idx);
        time_end = get_current_time();
        std::cout << "classnet cost time: " <<  (time_end - time_start)/1000.0  << "ms" << std::endl;

        f_result.open("./result.txt", std::ios::app);
        f_result << images.at(img_idx) << " " << idx_result << "\n";
        f_result.close();
    }
